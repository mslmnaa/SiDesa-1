                ...
                <div class="space-y-6">
                    ...
                    <div class="flex items-center justify-center p-4 bg-gray-50 rounded-xl">
                        <div class="flex items-center gap-2">
                            <div class="w-3 h-3 rounded-full {{ $product->status === 'active' ? 'bg-green-500' : 'bg-red-500' }}"></div>
                            <span class="text-base font-medium {{ $product->status === 'active' ? 'text-green-700' : 'text-red-700' }}">
                                {{ $product->status === 'active' ? '✅ Produk Tersedia' : '❌ Tidak Tersedia' }}
                            </span>
                        </div>
                    </div>
                        ...
                        <div class="space-y-4">
                            ...
                            @if($product->whatsapp_number)
                                <button type="button" onclick="openWhatsAppOrder('{{ $product->whatsapp_number }}', '{{ $product->name }}', '{{ number_format($product->price, 0, ',', '.') }}', '{{ route('products.show', $product) }}')" 
                                       class="w-full bg-gradient-to-r from-green-500 to-green-600 text-white py-4 px-6 rounded-xl font-semibold hover:from-green-600 hover:to-green-700 transition-all duration-200 flex items-center justify-center gap-3 shadow-lg">
                                ...
                                <div class="mt-2 text-center">
                                    <p class="text-sm text-gray-600">
                                        📞 {{ \App\Helpers\WhatsappHelper::getDisplayPhoneNumber($product->whatsapp_number) }}
                                    </p>
                                </div>
                            @else
                                <div class="text-center py-4 bg-gray-50 rounded-xl">
                                    <p class="text-gray-600">Nomor WhatsApp penjual belum tersedia</p>
                                </div>
                            @endif
                        </div>
                    @else
...
• Estimasi Total: Rp ${formattedTotalPrice}

🔗 Link produk: ${productUrl}

❓ Apakah produk ini masih tersedia?
📍 Bagaimana cara pemesanan dan pengirimannya?
