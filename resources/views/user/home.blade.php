...
<section class="relative bg-gradient-to-r from-green-600 to-green-800 text-white">
    <div class="max-w-7xl mx-auto px-4 py-20">
        <div class="text-center">
            ...
            <div class="space-x-4">
                <a href="{{ route('products.index') }}" class="bg-white text-green-600 px-8 py-3 rounded-lg font-semibold hover:bg-gray-100 transition-colors inline-block">
                    Lihat Produk
                </a>
                @guest
                    <a href="{{ route('register') }}" class="border-2 border-white text-white px-8 py-3 rounded-lg font-semibold hover:bg-white hover:text-green-600 transition-colors inline-block">
                        Daftar Sekarang
                    </a>
                @endguest
            </div>
...
<section class="py-16 bg-white">
    <div class="max-w-7xl mx-auto px-4">
        <div class="text-center mb-12">
            ...
            <p class="text-gray-600">Jelajahi berbagai kategori produk lokal kami</p>
        </div>

        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            @foreach($categories as $category)
                <a href="{{ route('products.category', $category) }}" class="group">
                    <div class="bg-gray-50 rounded-lg p-6 text-center hover:bg-green-50 transition-colors">
                        @if($category->image)
                        ...
                        <p class="text-sm text-gray-500 mt-1">{{ $category->products_count ?? $category->products->count() }} produk</p>
                    </div>
                </a>
            @endforeach
        </div>
...
<section class="py-16 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4">
        <div class="text-center mb-12">
            ...
            <p class="text-gray-600">Produk terpilih dari berbagai desa</p>
        </div>
...
<section class="py-16 bg-white">
    <div class="max-w-7xl mx-auto px-4">
        <div class="text-center max-w-3xl mx-auto">
            <h2 class="text-3xl font-bold text-gray-900 mb-4">{{ $aboutUs->title }}</h2>
            <p class="text-gray-600 text-lg leading-relaxed">{{ $aboutUs->content }}</p>
        </div>
    </div>
</section>
@endif
...
<section class="py-16 bg-green-600">
    <div class="max-w-7xl mx-auto px-4 text-center">
        ...
        <p class="text-green-100 text-lg mb-8">Dukung produk lokal dan ekonomi desa</p>
        <div class="space-x-4">
            @guest
                <a href="{{ route('register') }}" class="bg-white text-green-600 px-8 py-3 rounded-lg font-semibold hover:bg-gray-100 transition-colors inline-block">
                    Daftar Gratis
                </a>
                <a href="{{ route('products.index') }}" class="border-2 border-white text-white px-8 py-3 rounded-lg font-semibold hover:bg-white hover:text-green-600 transition-colors inline-block">
                    Jelajahi Produk
                </a>
            @else
                <a href="{{ route('products.index') }}" class="bg-white text-green-600 px-8 py-3 rounded-lg font-semibold hover:bg-gray-100 transition-colors inline-block">
